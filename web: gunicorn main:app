...
def register():
    if request.method == 'POST':
        ...
        db = get_db()
        user = db.execute("SELECT * FROM users WHERE email=? OR mobile=?", (email, mobile)).fetchone()
        ...
        db.execute("INSERT INTO users (fullname, email, mobile, password) VALUES (?, ?, ?, ?)",
